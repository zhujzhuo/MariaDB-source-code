...
Modified 3/9/2014 Jan Lindström jan.lindstrom@skysql.com
-- Removed bundled libarchive and quicklz, using conditional
   compilation if libraries installed on system and remove the
   features if they are not.
-- Removed gcrypt, gpg-error requirement, using conditional compilation
   if libraries installed on system and remove the features if
   they are not.
-- Removed direct usage of fil internal structures and replaced them with
   function calls
-- Removed features that are not yet supported by MariaDB
-- Use conditional compilation for posix routines
-- Modified XtraDB (and InnoDB) to compile this directly on
...
ulint flush_log_at_trx_commit = 2;
...
datafiles_iter_next(datafiles_iter_t *it)
{
	...
	fil_get_next_node(it->node, it->space, new_node, space);
	it->node = new_node;
	it->space = space;
...
#ifdef HAVE_LOG_CHECKSUM_ALGORITHM
...
#else
	msg("InnoDB: Using log block size 512\n");
#endif
...
xb_get_zip_size(os_file_t file)
{
	...
	success = os_file_read(file, page, 0, UNIV_PAGE_SIZE_MAX, FALSE);
	if (!success) {
		goto end;
	}
...
xtrabackup_copy_datafile(fil_node_t* node, uint thread_n)
{
	...
	fil_node_get_name_and_path(node, node_name, node_path);

	is_system = !fil_is_user_tablespace_id(fil_node_get_space_id(node));
...
#if defined(HAVE_POSIX_FADVISE) && defined(USE_POSIX_FADVISE)
...
retry:
	...
	success = os_file_read(src_file, log_buf, 0, LOG_FILE_HDR_SIZE, FALSE);
	if (!success) {
		goto error;
	}
	...
	ibool* 		success)	/* out: indicates error. TRUE = success */
{
	...
	*success = FALSE;
	...
	if (fil_space != NULL) {
		ulint fil_space_id = fil_space_get_space_id(fil_space);

		if (fil_space_id == space_id || space_id == ULINT_UNDEFINED) {
			...
			msg("xtrabackup: Renaming %s to %s.ibd\n",
				fil_space_name(fil_space), tmpname);

			if (!fil_rename_tablespace(NULL, fil_space_id,
						   tmpname, NULL))
			{
	...
	if (fil_space != NULL) {
		...
		msg("xtrabackup: Renaming %s to %s\n",
			fil_space_name(fil_space), dest_space_name);

		if (!fil_rename_tablespace(NULL, fil_space_get_space_id(fil_space), tmpname,
					   NULL))
		{
			msg("xtrabackup: Cannot rename %s to %s\n",
				fil_space_name(fil_space), dest_space_name);
	...
	void*		/*data*/)
{
	...
	while (!last_buffer) {
		...
		success = os_file_read(src_file, incremental_buffer,
			               offset, page_in_buffer * page_size, FALSE);
		if (!success) {
			goto error;
		}
					...
					this table */
{
	...
	fil_node_get_name_and_path((fil_node_t*)node, name, path);
...
skip_check:
	...
	while ((node = datafiles_iter_next(it)) != NULL) {
		...
		space = fil_node_get_space(node);

		/* Align space sizes along with fsp header. We want to process
		each space once, so skip all nodes except the first one in a
		multi-node space. */
		if (fil_chain_get_prev(node) != NULL) {
			continue;
		}

		mtr_start(&mtr);

		space_id = fil_space_get_space_id(space);

		mtr_s_lock(fil_space_get_latch(space_id, &flags), &mtr);

		block = buf_page_get(space_id,
		...
		fil_extend_space_to_desired_size(&actual_size, space_id, size);
	}
	...
	if (xtrabackup_export) {
		...
		while ((node = datafiles_iter_next(it)) != NULL) {
			...
			if (!fil_is_user_tablespace_id(fil_space_get_space_id(space))) {
				continue;
			}

			fil_node_get_name_and_path(node, node_name, node_path);
			/* node exist == file exist, here */
			strcpy(info_file_path, node_name);
