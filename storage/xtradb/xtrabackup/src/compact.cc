...
xb_expand_file(fil_node_t *node)
{
	...
	if (my_rename(tmpfile_path, name, MYF(MY_WME))) {
...
/*=====================*/
{
	...
	while ((node = datafiles_iter_next(it)) != NULL) {
		...
		if (!fil_is_user_tablespace_id(fil_node_get_space_id(node))) {
