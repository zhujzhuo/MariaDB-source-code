	...
	uint		thread_n)	/*!< thread number for diagnostics */
{
	...
	cursor->space_id = fil_node_get_space_id(node);
	cursor->is_system = !fil_is_user_tablespace_id(cursor->space_id);

	fil_node_get_name_and_path(node, name, path);

	strncpy(cursor->abs_path, name, sizeof(cursor->abs_path));
	...
	if (cursor->is_system || !srv_backup_mode) {
		...
		success = fil_system_open_node(node);
	...
	cursor->node = node;
	cursor->file = fil_node_get_handle(node);
	...
	if (srv_unix_file_flush_method == SRV_UNIX_O_DIRECT
		...
		os_file_set_nocache(cursor->file, name, "OPEN");
	}
