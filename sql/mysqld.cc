...
my_atomic_rwlock_t global_thread_id_lock;
...
void clean_up(bool print_message)
{
  ...
  */
  ...
  my_atomic_rwlock_destroy(&global_thread_id_lock);
...
void dec_thread_count(void)
{
  ...
  */
  ...
  /*
    ...
    above decrement call. We should only signal COND_thread_count if
    thread_count is likely to be 0. (false positives are ok)
  */
  if (!thread_count)
  {
    /* Signal close_connections() that all threads has died */
    mysql_mutex_lock(&LOCK_thread_count);
    mysql_cond_broadcast(&COND_thread_count);
    mysql_mutex_unlock(&LOCK_thread_count);
  }
...
/*
  ...
  This is mainly done to delete event threads which are not increasing
  global counters.
*/
