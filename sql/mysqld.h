...
{
  my_thread_id id;
  my_atomic_rwlock_wrlock(&global_thread_id_lock);
  id= my_atomic_addlong(&global_thread_id, 1);
  my_atomic_rwlock_wrunlock(&global_thread_id_lock);
  return (id);
}
...
extern void optimize_system_malloc(void);
